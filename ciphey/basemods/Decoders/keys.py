...
from ciphey.iface import Decoder, Config, X509, ParamSpec, registry, RSAPrivateKey, RSAPublicKey
...
_decoders = {
    "pem_private": ((lambda x: load_pem_private_key(x.encode(), None, backend=backend)), 0.01, str),
    "pem_public": ((lambda x: load_pem_public_key(x.encode(), backend=backend)), 0.01, str),
...
for name, (decoder, priority, src) in _decoders.items():
    t = types.new_class(
        name,
        (Decoder[src],),
